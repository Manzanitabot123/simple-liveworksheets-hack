    ...
    const CheckAnswers = function(arr, delay) {
        if (incr < arr.length) {
            ...
            if(arr[incr][0].includes('select:')) {
                ...
                select++; element.ariaChecked = true; console.log("%c"+incr+" | "+json.success+" => "+arr[incr][0], 'background: #005e06; color: white; font-size: 12px') 
            } else if(arr[incr][0].includes('tick:')) {
                element = document.querySelectorAll('[id^="tickbox"]')[tick];
                arr[incr][0].includes('yes')?element.click():{};
                tick++; element.ariaChecked = true; console.log("%c"+incr+" | "+json.success+" => "+arr[incr][0], 'background: #005e06; color: white; font-size: 12px') 
                ...
                choose++; element.ariaChecked = true; console.log("%c"+incr+" | "+json.success+" => "+arr[incr][0], 'background: #005e06; color: white; font-size: 12px') 
                ...
                text++; element.ariaChecked = true; console.log("%c"+incr+" | "+json.success+" => "+arr[incr][0], 'background: #005e06; color: white; font-size: 12px') 
