...
function worksheetHack(jsonlang) {
    function random(a) { return Math.floor(Math.random() * a) + 1 }
    let lg = navigator.language || navigator.userLanguage; 
    let json = jsonlang.lang.filter(d => (lg.includes(d.locale)))[0]
    if(!json) {json = jsonlang.lang.filter(d => (d.locale == 'en'))[0]};
    ...
    if(x[0].includes('select:')) {
        ...
        console.log("%c"+total+" | Success => "+x[0], 'background: #005e06; color: white; font-size: 12px') 
    } else if (x[0].includes('listen') && x[0].includes(':') || x[0].includes('joined:') || x[0].includes('drop:')) {
    } else if (x[0] =='') {
    ...
    if(nulos>0) {json.completed += " "+json.none.replace('<number>', nulos) }
    let speech = new SpeechSynthesisUtterance();
    speech.lang = json.locale;
    ...
    speech.volume = 1;
    speech.pitch = 1;
    speech.voice = voices[0]; speech.text = json.completed;
    window.speechSynthesis.speak(speech)
}

fetch("https://raw.githubusercontent.com/Manzanitabot123/simple-liveworksheets-hack/script/languages/languages.json").then(a=>a.json().then(b=>{worksheetHack(b)}));